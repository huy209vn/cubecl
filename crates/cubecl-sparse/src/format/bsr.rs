...
pub struct BsrStorage {
    pub block_row_ptrs: Handle,
    pub block_col_indices: Handle,
...
impl SparseFormat for BsrStorage {
    const FORMAT_ID: SparseFormatId = SparseFormatId::Bsr { block_rows: 16, block_cols: 16 };
    const ROW_MAJOR: bool = true;
    const COL_MAJOR: bool = false;
    const DYNAMIC_PATTERN: bool = false;
}

impl SparseStorage for BsrStorage {
    type Metadata = BsrMetadata;
...
impl SparseMetadata for BsrMetadata {
    fn shape(&self) -> &[usize] {
        todo!()
    }
